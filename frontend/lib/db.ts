...
export interface Interview {
  ...
  round1Evaluation?: any;
  round1Evidence?: any;
  round2Evaluation?: any;
  round2Evidence?: any;
  round3Evaluation?: any;
  round3Evidence?: any;
...
const defaultDb: Database = {
  companies: [
    { id: 'c1', name: 'Plantra Labs', industry: 'AI Infrastructure & Developer Platform' }
  ],
  jobs: [
    {
      id: 'j1',
...
/**
 * Universal interview resolver that safely locates or auto-seeds an Interview record
 * by interview ID, application ID, blueprint ID, or fallback demo ID.
 */
export function resolveInterview(db: Database, id: string): Interview {
  if (!id) id = 'demo-interview-test';

  // 1. Direct match on interview.id
  let interview = db.interviews.find(i => i.id === id);
  if (interview) return interview;

  // 2. Match on applicationId
  interview = db.interviews.find(i => i.applicationId === id);
  if (interview) return interview;

  // 3. Match on blueprint.id or blueprint.interviewId
  const blueprint = db.blueprints.find(b => b.id === id || b.interviewId === id);
  if (blueprint) {
    interview = db.interviews.find(i => i.id === blueprint.interviewId || i.applicationId === blueprint.interviewId);
    if (interview) return interview;
  }

  // 4. Resolve candidate application (target ID or fallback)
  const application = db.applications.find(a => a.id === id) || db.applications[db.applications.length - 1] || db.applications[0];
  const appId = application ? application.id : 'app1';

  // 5. Check if an interview exists for this application
  interview = db.interviews.find(i => i.applicationId === appId);
  if (interview) {
    // If queried by a specific custom ID, alias it or return existing
    return interview;
  }

  // 6. Create and persist new Interview record
  const newInterview: Interview = {
    id: id.startsWith('int_') || id.startsWith('demo') ? id : `int_${id}`,
    applicationId: appId,
